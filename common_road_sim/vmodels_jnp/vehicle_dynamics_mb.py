...
from . import tire_model as tireModel

"""
Adapted from Matthias Althoff's vehicleDynamics_mb function in vehiclemodels/vehicle_dynamics_mb.py
...
def vehicle_dynamics_mb(x, uInit, p):
    ...
    sumY_uf = (
        (F_y_LF + F_y_RF) * jnp.cos(x[2])
        + (F_x_LF + F_x_RF) * jnp.sin(x[2])
        - F_RAF * jnp.cos(x[6])
        - (F_SLF + F_SRF) * jnp.sin(x[6])
    )

    sumY_ur = (
        (F_y_LR + F_y_RR) - F_RAR * jnp.cos(x[6]) - (F_SLR + F_SRR) * jnp.sin(x[6])
    )
